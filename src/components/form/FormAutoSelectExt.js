...
export default function FormAutoSelectExt({children, name, dataUrl, syncFields = [], valueField = "id", labelField}){
    ...
    const handleSelectChange = (e) => {
        ...
        const selectedItem = items.find(item => String(getNestedValue(item, valueField)) === selectedId);
    ...
    return (
        <div className="mb-3 form-floating">
            ...
            >
                ...
                {items.map(item => (
                    <option className='text-xs' value={getNestedValue(item, valueField)} key={getNestedValue(item, valueField)}>
                        {getNestedValue(item, labelField)}
                    </option>
                ))}

            </select>
            <label className='form-label'>{capitalizar(name)}</label>
        </div>
